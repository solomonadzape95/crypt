    ...
    <main className="flex-1 flex flex-col">
      <header className="relative border-b border-[var(--rule-0)] bg-[var(--ink-1)]">
        ...
        <div className="max-w-[88rem] mx-auto w-full px-6 md:px-12 h-14 flex items-center justify-between">
          ...
          <Link href="/vaults" className="label hover:text-[var(--fg-0)] transition-colors">
            dashboard →
          </Link>
        </div>
      </header>
      ...
      <section className="max-w-3xl mx-auto w-full px-6 py-16 flex flex-col gap-12">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-[15px] text-[var(--fg-1)] leading-relaxed max-w-[68ch]">
            Two people put money into a lockbox on Stellar. A bot watches an API. If
            the API stays down past the limit, the money goes to the subscriber. If it
            stays up, the money goes to the provider. Nobody decides — the contract
            does, based on what the bot saw.
          </p>
        </div>
        ...
        <p className="text-[13px] text-[var(--fg-2)] leading-relaxed max-w-[68ch]">
          The amounts are locked into the contract at deposit time. The contract only
          knows two addresses — the provider's payout wallet and the subscriber's
          payout wallet. We can decide which one wins, but we can't change either
          address or take a cut.
        </p>

        <LabeledRule label="04 · what's next" />
        <p className="text-[14px] text-[var(--fg-1)] leading-relaxed max-w-[68ch]">
          Today we run the API check ourselves — you have to trust we report honestly.
          Next we'll have multiple independent checkers agreeing before a payout
          triggers. After that, a math-based proof that the checkers agreed, verified
          by the contract itself. The goal: nobody — us included — can decide who gets
          paid. The math does.
        </p>
      </section>
    </main>
...
function Step({
  ...
  return (
    <div className="grid grid-cols-[3rem_1fr] gap-4 px-4 py-4 border-b border-[var(--rule-0)] last:border-b-0">
      <span className="label text-[var(--amber)] numeric self-start mt-[2px]">{n}</span>
